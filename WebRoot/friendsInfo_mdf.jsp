...
function test() {
  ...
  %>
	...
	<%} %>
     <h1> 我的好友录</h1>
     <div  style="position:relative;left:128px;top:-32px"><a id="mdf_link" class="button button-little bg-red"  href="javascript:void(0)"><span class="icon-power-off">
     </span> 修改选中好友</a></div>
  ...
  <form action="delete" method="post" name="mydelete">
    ...
    if(pageNo>1){
    %>
    	<a href = "friendsInfo_mdf.jsp?pageNo=<%=pageNo-1%>">[上一页]</a>
 	<%
    }
    // 当前页
    for(int i =1;i<=pageCount;i++){
    	...
    	%>
    		<a href ="friendsInfo_mdf.jsp?pageNo=<%=i%>">[<%=i %>]</a>
    	<%
    		}
    }
    if(pageNo<pageCount){
    	%>
    	<a href = "friendsInfo_mdf.jsp?pageNo=<%=pageNo+1%>">[下一页]</a>
    <%	
    }
    %>

...
function test() {
  ...
  %>
		<h2>你还没与权限查看此界面，请先<a href = "login.jsp">登录</a></h2>
	<%} %>
     <h1> 我的好友录</h1>
     <div  style="position:relative;left:128px;top:-32px"><a id="out" class="button button-little bg-red" onclick="check_del()" href="javascript:void(0)"><span class="icon-power-off">
     </span> 删除选中好友</a></div>
  ...
  <form action="delete" method="post" name="mydelete">
    ...
    if(pageNo>1){
    %>
    	<a href = "friendsInfo_del.jsp?pageNo=<%=pageNo-1%>">[上一页]</a>
 	<%
    }
    // 当前页
    for(int i =1;i<=pageCount;i++){
    	...
    	%>
    		<a href ="friendsInfo_del.jsp?pageNo=<%=i%>">[<%=i %>]</a>
    	<%
    		}
    }
    if(pageNo<pageCount){
    	%>
    	<a href = "friendsInfo_del.jsp?pageNo=<%=pageNo+1%>">[下一页]</a>
    <%	
    }
    %>
